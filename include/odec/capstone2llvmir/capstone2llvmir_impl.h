...
#ifndef ODEC_CAPSTONE2LLVMIR_IMPL_H
#define ODEC_CAPSTONE2LLVMIR_IMPL_H

#include "odec/capstone2llvmir/capstone2llvmir.h"
#include "odec/capstone2llvmir/llvmir_utils.h"
...
public:
    Capstone2LlvmIrTranslator_impl(cs_arch a, cs_mode basic, cs_mode extra,
                                   llvm::Module *m);
    ...
    //
public:
    virtual TranslationResult translate(const uint8_t *bytes, std::size_t size,
                                        odec::common::Address a,
                                        llvm::IRBuilder<> &irb,
                                        std::size_t count = 0,
...
public:
    ...
    virtual uint32_t getCapstoneRegister(
...
protected:
     ...
     * param to.
     */
    llvm::Type *_checkTypeConversion(llvm::IRBuilder<> &irb, llvm::Type *to,
                                     eOpConv ct);
    //
    //==============================================================================
...
protected:
     ...
     * @return Loaded value.
     */
    virtual llvm::Value *loadRegister(uint32_t r, llvm::IRBuilder<> &irb,
                                      llvm::Type *dstType = nullptr,
                                      eOpConv ct = eOpConv::THROW) = 0;

    virtual llvm::Value *loadOp(CInsnOp &op, llvm::IRBuilder<> &irb,
                                llvm::Type *ty = nullptr, bool lea = false) = 0;

    virtual llvm::Instruction *storeRegister(
        uint32_t r, llvm::Value *val, llvm::IRBuilder<> &irb,
        eOpConv ct = eOpConv::SEXT_TRUNC_OR_BITCAST) = 0;

    virtual llvm::Instruction *storeOp(
        CInsnOp &op, llvm::Value *val, llvm::IRBuilder<> &irb,
        eOpConv ct = eOpConv::SEXT_TRUNC_OR_BITCAST) = 0;
     ...
     */
    llvm::Value *loadOp(CInsn *ci, llvm::IRBuilder<> &irb, std::size_t idx,
     ...
     * @param loadType	Type of loaded value. (not relevant if nullptr)
     * @param dstType	Desired type of loaded value (not changed if nullptr).
     * @param ct		Used conversion. Defaultly NOTHING as "do not
     * convert".
     */
    std::vector<llvm::Value *> _loadOps(CInsn *ci, llvm::IRBuilder<> &irb,
                                        std::size_t opCnt,
    ...
    std::vector<llvm::Value *> _loadOpsUniversal(
        CInsn *ci, llvm::IRBuilder<> &irb, std::size_t opCnt,
        bool strictCheck = true, eOpConv ict = eOpConv::SEXT_TRUNC_OR_BITCAST,
        eOpConv fct = eOpConv::FPCAST_OR_BITCAST);

    llvm::Value *loadOpUnary(CInsn *ci, llvm::IRBuilder<> &irb,
                             llvm::Type *dstType = nullptr,
                             llvm::Type *loadType = nullptr,
                             eOpConv ct = eOpConv::THROW);

    std::pair<llvm::Value *, llvm::Value *> loadOpBinary(
        CInsn *ci, llvm::IRBuilder<> &irb, eOpConv ct = eOpConv::NOTHING);
    ...
    std::pair<llvm::Value *, llvm::Value *> loadOpBinary(
        ...
        llvm::Type *dstType = nullptr, eOpConv ct = eOpConv::NOTHING);

    llvm::Value *loadOpBinaryOp0(CInsn *ci, llvm::IRBuilder<> &irb,
                                 llvm::Type *ty = nullptr);

    llvm::Value *loadOpBinaryOp1(CInsn *ci, llvm::IRBuilder<> &irb,
                                 llvm::Type *ty = nullptr);
    ...
    std::pair<llvm::Value *, llvm::Value *> loadOpBinaryOrTernaryOp1Op2(
        CInsn *ai, llvm::IRBuilder<> &irb, eOpConv ct = eOpConv::NOTHING);

    std::pair<llvm::Value *, llvm::Value *> loadOpBinaryOrTernaryOp1Op2(
        CInsn *ai, llvm::IRBuilder<> &irb, eOpConv ict, eOpConv fct);
    ...
    //
protected:
    llvm::Value *generateCarryAdd(llvm::Value *add, llvm::Value *op0,
                                  llvm::IRBuilder<> &irb);

    llvm::Value *generateCarryAddC(llvm::Value *op0, llvm::Value *op1,
                                   llvm::IRBuilder<> &irb,
                                   llvm::Value *cf = nullptr);

    llvm::Value *generateCarryAddInt4(llvm::Value *op0, llvm::Value *op1,
                                      llvm::IRBuilder<> &irb);

    llvm::Value *generateCarryAddCInt4(llvm::Value *op0, llvm::Value *op1,
                                       llvm::IRBuilder<> &irb,
                                       llvm::Value *cf = nullptr);

    llvm::Value *generateOverflowAdd(llvm::Value *add, llvm::Value *op0,
                                     llvm::Value *op1, llvm::IRBuilder<> &irb);

    llvm::Value *generateOverflowAddC(llvm::Value *add, llvm::Value *op0,
                                      llvm::Value *op1, llvm::IRBuilder<> &irb,
                                      llvm::Value *cf = nullptr);

    llvm::Value *generateOverflowSub(llvm::Value *sub, llvm::Value *op0,
                                     llvm::Value *op1, llvm::IRBuilder<> &irb);

    llvm::Value *generateOverflowSubC(llvm::Value *sub, llvm::Value *op0,
                                      llvm::Value *op1, llvm::IRBuilder<> &irb,
                                      llvm::Value *cf = nullptr);

    llvm::Value *generateBorrowSub(llvm::Value *op0, llvm::Value *op1,
                                   llvm::IRBuilder<> &irb);

    llvm::Value *generateBorrowSubC(llvm::Value *sub, llvm::Value *op0,
                                    llvm::Value *op1, llvm::IRBuilder<> &irb,
                                    llvm::Value *cf = nullptr);

    llvm::Value *generateBorrowSubInt4(llvm::Value *op0, llvm::Value *op1,
                                       llvm::IRBuilder<> &irb);

    llvm::Value *generateBorrowSubCInt4(llvm::Value *op0, llvm::Value *op1,
                                        llvm::IRBuilder<> &irb,
                                        llvm::Value *cf = nullptr);
    //
    //==============================================================================
...
protected:
    ...
    llvm::Function *getPseudoAsmFunction(cs_insn *insn, llvm::Type *retType,
                                         llvm::ArrayRef<llvm::Type *> params,
                                         const std::string &name = "");

    // Unary.
    void translatePseudoAsmOp0Fnc(cs_insn *i, CInsn *ci,
                                  llvm::IRBuilder<> &irb);

    void translatePseudoAsmFncOp0(cs_insn *i, CInsn *ci,
                                  llvm::IRBuilder<> &irb);

    void translatePseudoAsmOp0FncOp0(cs_insn *i, CInsn *ci,
                                     llvm::IRBuilder<> &irb);

    // Binary.
    void translatePseudoAsmFncOp0Op1(cs_insn *i, CInsn *ci,
                                     llvm::IRBuilder<> &irb);

    void translatePseudoAsmOp0FncOp1(cs_insn *i, CInsn *ci,
                                     llvm::IRBuilder<> &irb);

    void translatePseudoAsmOp0FncOp0Op1(cs_insn *i, CInsn *ci,
                                        llvm::IRBuilder<> &irb);

    // Ternary.
    void translatePseudoAsmFncOp0Op1Op2(cs_insn *i, CInsn *ci,
                                        llvm::IRBuilder<> &irb);

    void translatePseudoAsmOp0FncOp1Op2(cs_insn *i, CInsn *ci,
                                        llvm::IRBuilder<> &irb);

    void translatePseudoAsmOp0FncOp0Op1Op2(cs_insn *i, CInsn *ci,
                                           llvm::IRBuilder<> &irb);

    // Quaternary.
    void translatePseudoAsmFncOp0Op1Op2Op3(cs_insn *i, CInsn *ci,
                                           llvm::IRBuilder<> &irb);

    void translatePseudoAsmOp0FncOp1Op2Op3(cs_insn *i, CInsn *ci,
                                           llvm::IRBuilder<> &irb);

    void translatePseudoAsmOp0FncOp0Op1Op2Op3(cs_insn *i, CInsn *ci,
                                              llvm::IRBuilder<> &irb);

    void translatePseudoAsmOp0Op1FncOp0Op1Op2Op3(cs_insn *i, CInsn *ci,
                                                 llvm::IRBuilder<> &irb);
    ...
    virtual void translatePseudoAsmGeneric(cs_insn *i, CInsn *ci,
                                           llvm::IRBuilder<> &irb);
...
#endif  // ODEC_CAPSTONE2LLVMIR_IMPL_H
