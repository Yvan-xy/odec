...
#ifndef ODEC_CONVERSION_H
#define ODEC_CONVERSION_H
 ...
 */
template <typename N>
void bytesToHexString(const N *data, std::size_t dataSize, std::string &result,
                      std::size_t offset = 0, std::size_t size = 0,
                      bool uppercase = true, bool spacing = false) {
 ...
 */
template <typename N>
void bytesToHexString(const std::vector<N> &bytes, std::string &result,
                      std::size_t offset = 0, std::size_t size = 0,
                      bool uppercase = true, bool spacing = false) {
    bytesToHexString(bytes.data(), bytes.size(), result, offset, size,
                     uppercase, spacing);
}
...
std::string intToHexString(I w, bool addBase = false, unsigned fillToN = 0) {
    ...
    for (size_t i = 0; i < rc.size(); ++i) {
        if (started) {
            res[j++] = rc[i];
        } else if (rc[i] != '0' || (rc.size() - i <= fillToN) ||
                   (i == rc.size() - 1)) {
            res[j++] = rc[i];
            started = true;
        }
 ...
 */
template <typename N>
void bytesToString(const N *data, std::size_t dataSize, std::string &result,
                   std::size_t offset = 0, std::size_t size = 0) {
    if (!data) {
        dataSize = 0;
    }
    ...
    if (offset >= dataSize) {
        ...
        size =
            (size == 0 || offset + size > dataSize) ? dataSize - offset : size;
    }
 ...
 */
template <typename N>
void bytesToString(const std::vector<N> &bytes, std::string &result,
                   std::size_t offset = 0, std::size_t size = 0) {
