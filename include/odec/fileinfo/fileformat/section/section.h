...
#ifndef ODEC_SECTION_H
#define ODEC_SECTION_H
 ...
 */
    ...
    public:
        ...
        const llvm::StringRef getBytes(unsigned long long sOffset = 0,
                                       unsigned long long sSize = 0) const;
        ...
        NumberType getBytesAtOffsetAsNumber(unsigned long long sOffset) const {
            if (bytes.size() < sizeof(NumberType) ||
                sOffset > bytes.size() - sizeof(NumberType)) {
                return {};
            }
        ...
        bool getBytes(std::vector<unsigned char> &sResult,
                      unsigned long long sOffset = 0,
                      unsigned long long sSize = 0) const;

        bool getString(std::string &sResult, unsigned long long sOffset = 0,
                       unsigned long long sSize = 0) const;
