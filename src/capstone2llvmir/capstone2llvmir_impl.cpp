...
#include "odec/capstone2llvmir/capstone2llvmir_impl.h"
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::Capstone2LlvmIrTranslator_impl(
    cs_arch a, cs_mode basic, cs_mode extra, llvm::Module *m)
    : _arch(a),
      _basicMode(basic),
      _extraMode(extra),
...
void Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::setIgnoreUnexpectedOperands(bool f) {
    _ignoreUnexpectedOperands = f;
}

template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::setIgnoreUnhandledInstructions(bool f) {
    _ignoreUnhandledInstructions = f;
}

template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::setGeneratePseudoAsmFunctions(bool f) {
    _generatePseudoAsmFunctions = f;
}

template <typename CInsn, typename CInsnOp>
bool Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::isIgnoreUnexpectedOperands() const {
    return _ignoreUnexpectedOperands;
}

template <typename CInsn, typename CInsnOp>
bool Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::isIgnoreUnhandledInstructions() const {
    return _ignoreUnhandledInstructions;
}

template <typename CInsn, typename CInsnOp>
bool Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::isGeneratePseudoAsmFunctions() const {
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::translate(
    const uint8_t *bytes, std::size_t size, odec::common::Address a,
    llvm::IRBuilder<> &irb, std::size_t count, bool stopOnBranch) {
    ...
    while (disasmRes) {
        ...
        // Update translated binary chunk size
        ...
        // TODO: hack, solve better.
        disasmRes = cs_disasm_iter(_handle, &bytes, &size, &address, insn);
        if (!disasmRes && _arch == CS_ARCH_MIPS &&
            _basicMode == CS_MODE_MIPS32) {
            modifyBasicMode(CS_MODE_MIPS64);
            disasmRes = cs_disasm_iter(_handle, &bytes, &size, &address, insn);
            modifyBasicMode(CS_MODE_MIPS32);
        }
...
template <typename CInsn, typename CInsnOp>
llvm::GlobalVariable *Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::getAsm2LlvmMapGlobalVariable() const {
    return _asm2llvmGv;
}
...
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isCallFunctionCall(
    ...
    if (c) {
        return isCallFunction(c->getCalledFunction());
    } else {
        return false;
    }
}
...
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isReturnFunctionCall(
    llvm::CallInst *c) const {
    if (c) {
        return isReturnFunction(c->getCalledFunction());
    } else {
        return false;
    }
}
...
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isBranchFunctionCall(
    llvm::CallInst *c) const {
    if (c) {
        return isBranchFunction(c->getCalledFunction());
    } else {
        return false;
    }
}
...
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isCondBranchFunctionCall(
    llvm::CallInst *c) const {
    if (c) {
        return isCondBranchFunction(c->getCalledFunction());
    } else {
        return false;
    }
}
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isRegister(
    llvm::Value *v) const {
    auto it = _llvm2CapstoneRegs.find(
        llvm::dyn_cast_or_null<llvm::GlobalVariable>(v));
...
bool Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::isPseudoAsmFunctionCall(
    llvm::CallInst *c) const {
    if (c) {
        return isPseudoAsmFunction(c->getCalledFunction());
    } else {
        return false;
    }
}
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::generateSpecialAsm2LlvmMapGlobal() {
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateSpecialAsm2LlvmInstr(
    llvm::IRBuilder<> &irb, cs_insn *i) {
    odec::common::Address a = i->address;
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateCallFunctionCall(
    llvm::IRBuilder<> &irb, llvm::Value *t) {
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateCondCallFunctionCall(
    llvm::IRBuilder<> &irb, llvm::Value *cond, llvm::Value *t) {
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::createRegister(
    uint32_t r, llvm::GlobalValue::LinkageTypes lt,
...
llvm::Value *Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::loadOp(
    CInsn *ci, llvm::IRBuilder<> &irb, std::size_t idx, llvm::Type *loadType,
    llvm::Type *dstType, eOpConv ct) {
    if (ci->op_count <= idx) {
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::_loadOps(
    CInsn *ci, llvm::IRBuilder<> &irb, std::size_t opCnt, bool strict,
    llvm::Type *loadType, llvm::Type *dstType, eOpConv ct) {
...
llvm::Value *Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::loadOpUnary(
    CInsn *ci, llvm::IRBuilder<> &irb, llvm::Type *loadType,
    llvm::Type *dstType, eOpConv ct) {
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::loadOpBinary(
    CInsn *ci, llvm::IRBuilder<> &irb, eOpConv ct) {
...
std::pair<llvm::Value *, llvm::Value *>
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::loadOpBinary(
    CInsn *ci, llvm::IRBuilder<> &irb, eOpConv ict, eOpConv fct) {
...
llvm::Value *Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::loadOpBinaryOp0(
    CInsn *ci, llvm::IRBuilder<> &irb, llvm::Type *ty) {
...
llvm::Value *Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateCarryAdd(
    llvm::Value *add, llvm::Value *op0, llvm::IRBuilder<> &irb) {
...
llvm::Value *Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateCarryAddC(
    llvm::Value *op0, llvm::Value *op1, llvm::IRBuilder<> &irb,
    llvm::Value *cf) {
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateOverflowAddC(
    llvm::Value *add, llvm::Value *op0, llvm::Value *op1,
    llvm::IRBuilder<> &irb, llvm::Value *cf) {
    if (cf == nullptr) {
        cf = loadRegister(getCarryRegister(), irb);
    }
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateOverflowSub(
    llvm::Value *sub, llvm::Value *op0, llvm::Value *op1,
    llvm::IRBuilder<> &irb) {
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getPseudoAsmFunction(
    cs_insn *insn, llvm::FunctionType *type, const std::string &name) {
    ...
    if (fIt == _insn2asmFunctions.end()) {
        auto *fnc = llvm::Function::Create(
            ...
            name.empty() ? getPseudoAsmFunctionName(insn) : name, _module);
...
llvm::Function *
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::getPseudoAsmFunction(
    cs_insn *insn, llvm::Type *retType, llvm::ArrayRef<llvm::Type *> params,
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::generateTypeConversion(
    llvm::IRBuilder<> &irb, llvm::Value *from, llvm::Type *to, eOpConv ct) {
    ...
    switch (ct) {
        case eOpConv::SEXT_TRUNC_OR_BITCAST: {
            if (!to->isIntegerTy()) {
                throw GenericError(
                    "Invalid combination of conversion method and destination "
                    "type");
            }
            ...
            if (from->getType()->isIntegerTy()) {
                ...
                auto size = _module->getDataLayout().getTypeStoreSizeInBits(
                    from->getType());
        ...
        case eOpConv::FPCAST_OR_BITCAST: {
            ...
            if (from->getType()->isFloatingPointTy()) {
                ...
                auto isize = _module->getDataLayout().getTypeStoreSizeInBits(
                    from->getType());
                auto dsize = _module->getDataLayout().getTypeStoreSizeInBits(
                    irb.getDoubleTy());
                auto fsize = _module->getDataLayout().getTypeStoreSizeInBits(
                    irb.getFloatTy());
...
Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::_checkTypeConversion(
    llvm::IRBuilder<> &irb, llvm::Type *to, eOpConv ct) {
    switch (ct) {
        case eOpConv::ZEXT_TRUNC_OR_BITCAST:
        case eOpConv::SEXT_TRUNC_OR_BITCAST: {
            if (!to->isIntegerTy()) {
...
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::translatePseudoAsmOp0Fnc(
    cs_insn *i, CInsn *ci, llvm::IRBuilder<> &irb) {
    EXPECT_IS_UNARY(i, ci, irb);

    llvm::Function *fnc = getPseudoAsmFunction(i, getDefaultType(),
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::translatePseudoAsmOp0FncOp0(cs_insn *i, CInsn *ci,
                                                 llvm::IRBuilder<> &irb) {
    EXPECT_IS_UNARY(i, ci, irb);
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::translatePseudoAsmFncOp0Op1(cs_insn *i, CInsn *ci,
                                                 llvm::IRBuilder<> &irb) {
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::translatePseudoAsmOp0FncOp1(cs_insn *i, CInsn *ci,
                                                 llvm::IRBuilder<> &irb) {
    EXPECT_IS_BINARY(i, ci, irb);
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::translatePseudoAsmOp0FncOp0Op1(cs_insn *i, CInsn *ci,
                                                    llvm::IRBuilder<> &irb) {
    EXPECT_IS_BINARY(i, ci, irb);
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::translatePseudoAsmFncOp0Op1Op2(cs_insn *i, CInsn *ci,
                                                    llvm::IRBuilder<> &irb) {
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::translatePseudoAsmOp0FncOp1Op2(cs_insn *i, CInsn *ci,
                                                    llvm::IRBuilder<> &irb) {
    EXPECT_IS_TERNARY(i, ci, irb);
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::translatePseudoAsmOp0FncOp0Op1Op2(cs_insn *i, CInsn *ci,
                                                       llvm::IRBuilder<> &irb) {
    EXPECT_IS_TERNARY(i, ci, irb);
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::translatePseudoAsmFncOp0Op1Op2Op3(cs_insn *i, CInsn *ci,
                                                       llvm::IRBuilder<> &irb) {
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<
    CInsn, CInsnOp>::translatePseudoAsmOp0FncOp1Op2Op3(cs_insn *i, CInsn *ci,
                                                       llvm::IRBuilder<> &irb) {
    EXPECT_IS_QUATERNARY(i, ci, irb);
...
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    translatePseudoAsmOp0FncOp0Op1Op2Op3(cs_insn *i, CInsn *ci,
                                         llvm::IRBuilder<> &irb) {
    EXPECT_IS_QUATERNARY(i, ci, irb);
...
template <typename CInsn, typename CInsnOp>
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::
    translatePseudoAsmOp0Op1FncOp0Op1Op2Op3(cs_insn *i, CInsn *ci,
                                            llvm::IRBuilder<> &irb) {
    EXPECT_IS_QUATERNARY(i, ci, irb);
...
void Capstone2LlvmIrTranslator_impl<CInsn, CInsnOp>::throwUnexpectedOperands(
    cs_insn *i, const std::string comment) {
