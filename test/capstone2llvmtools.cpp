...
#include "odec/utils/io/log.h"

#include "odec/capstone2llvmir/capstone2llvmir.h"

using namespace odec::utils::io;

// byte ptr [0x12345678], 0x11
std::vector<uint8_t> CODE = odec::utils::hexStringToBytes("80 05 78 56 34 12 11 00");
...
public:
    ...
    ProgramOptions(int argc, char *argv[]) {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (c == "-a") {
                ...
                if (!odec::utils::strToNum(_base, base, std::hex)) {
                    printHelpAndDie();
                }
            } else if (c == "-c") {
                _code = getParamOrDie(argc, argv, i);
                code = odec::utils::hexStringToBytes(_code);
            } else if (c == "-t") {
    ...
    cs_mode getDefaultBasicModeFromArch(cs_arch a) {
        switch (a) {
            case CS_ARCH_ARM:
                return CS_MODE_ARM; // CS_MODE_THUMB
            case CS_ARCH_ARM64:
                return CS_MODE_ARM;
            case CS_ARCH_MIPS:
                return CS_MODE_MIPS32; // CS_MODE_MIPS{32, 64, 32R6}
            case CS_ARCH_X86:
                return CS_MODE_32; // CS_MODE_{16, 32, 64}
            case CS_ARCH_PPC:
                return CS_MODE_32;
            case CS_ARCH_SPARC:
                return CS_MODE_LITTLE_ENDIAN; // 0
            case CS_ARCH_SYSZ:
    ...
    void dump() {
        std::string tmp;
        odec::utils::bytesToHexString(code, tmp, 0, 0, false, true);
        Log::info() << std::endl;
...
using namespace odec::capstone2llvmir;
